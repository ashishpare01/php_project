...
if (!$fileid) {
    // Handle error: unable to get file ID
    $fileid = 1; // Set a default value
    // die("Error: Unable to get file ID");
}
		...
		if ($picture == "") {
			...
			if (isset($_SESSION['APPLICANTID'])) {
				$file_id = date('Y') . $fileid->AUTO;
				$sql = "INSERT INTO `tblattachmentfile` (FILEID, `USERATTACHMENTID`, `FILE_NAME`, `FILE_LOCATION`, `JOBID`) 
						VALUES ('$file_id', '{$_SESSION['APPLICANTID']}', 'Resume', '{$location}', '{$jobid}')";
				$mydb->setQuery($sql);  
				doUpdate($jobid, $fileid->AUTO);
			} else {			 
				$applicant_id = date('Y') . $applicantid->AUTO;
				$file_id = date('Y') . $fileid->AUTO;
				$sql = "INSERT INTO `tblattachmentfile` (FILEID, `USERATTACHMENTID`, `FILE_NAME`, `FILE_LOCATION`, `JOBID`) 
						VALUES ('$file_id', '$applicant_id', 'Resume', '{$location}', '{$jobid}')";
				$mydb->setQuery($sql);  			 
